...
class Parametres: #Classe comportant les différents paramètres pour le Jeu
    ...
    taille_fenetre = (taille_plateau * taille_cellule + (taille_plateau + 1) * 2 + 200, taille_plateau * taille_cellule + (taille_plateau + 1) * 2)
...
class Pion: #Classe comportant tous les méthodes et fonctions relatives aux Pions
    ...
    def dessiner(self): #Fonction qui permet de dessiner les pions ou les redessiner en cas de déplacement
        ...
        pygame.draw.circle(Parametres.fenetre, self.couleur, (self.x * c + c//2 + (self.x+1) * 2, self.y * c + c//2 + (self.y+1) * 2), c//3) #Dessine un cercle qui représente le Pion
...
class Barriere: # Classe comportant tous les méthodes et fonctions relatives aux Barrières
    ...
    def dessiner(self): # Fonction pour dessiner les barrières
        ...
        if self.orientation == "horizontal":
            pygame.draw.rect(f, self.couleur, ((self.x*c + (self.x+1) * 2, self.y*c + (self.y+1) * 2), (c*2, epaisseur))) #Dessine la barrière Horizontalement
        else:  # "vertical"
...
joueur_jaune = Joueur(pion_jaune, barrieres_par_joueur, Parametres.JAUNE, "Jaune") if Parametres.nb_players == 4 else None
...
class Jeu: #Classe comportant tous les méthodes et fonctions relatives au Jeu
    def dessiner_plateau(): # Dessiner le plateau de jeu
        ...
        pygame.draw.rect(Parametres.fenetre, Parametres.GRIS, (Parametres.taille_fenetre[0]-200, 0, 200, Parametres.taille_fenetre[1]))
    ...
    while en_cours:
        ...
        text_barrieres = font.render("Barrières restantes : " + str(joueurs[joueur_en_cours].barrieres_restantes), True, Parametres.NOIR)
        Parametres.fenetre.blit(text_barrieres, (Parametres.taille_fenetre[0] - 190, 40))

        # Vérification de la victoire
        for joueur in joueurs:
            victoire_texte = font.render("Bravo joueur " + joueurs[joueur_en_cours-1].couleur_nom + ", vous avez gagné !", True, joueurs[joueur_en_cours-1].couleur)
